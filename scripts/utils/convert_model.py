...
def convert_model(
    ...
    # Convert the model
    print(
        f"Converting to {format_type} format with {compression_method} compression (level {compression_level})..."
    )
    start_time = time.time()
...
def main():
    ...
    parser.add_argument(
        "--output", type=str, required=True, help="Path to save the converted model to"
    )
    parser.add_argument(
        "--format",
        type=str,
