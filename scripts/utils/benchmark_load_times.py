...
def measure_load_time(file_path: Path, iterations: int = 5) -> Dict[str, Any]:
...
def measure_tokenize_time(file_path: Path, text: str, iterations: int = 5) -> Dict[str, Any]:
...
def run_benchmark(model_path: Path, output_dir: Path, text: str) -> None:
    ...
    for format_type, compression, level, filename in formats:
        ...
        if format_type == "binary":
            save_binary_model(source_data, test_path, compression_method=compression, level=level)
        else:
            save_compressed_json(
                source_data, test_path, level=level, use_compression=(format_type == "json_xz")
            )
    ...
    print(
        f"{'Format':<10} {'Compression':<12} {'Level':<6} {'Size':<10} {'Mean (s)':<10} {'Median (s)':<10} {'Relative':<10}"
    )
    print("-" * 80)
    ...
    for format_type, compression, level, path in test_files:
        ...
        print(
            f"{format_type:<10} {compression:<12} {level:<6} {format_size(size):<10} "
