...
def test_model_accuracy(tokenizer: PunktSentenceTokenizer, texts: List[str], include_warmup: bool = False) -> Dict[str, Any]:
    """
...
def benchmark_only():
    ...
    if test_path.exists():
        ...
        speed_improvement = ((avg_warm_speed - avg_cold_speed) / avg_cold_speed * 100) if avg_cold_speed > 0 else 0
        
        # Print detailed results
        print("\n=== Performance Comparison (Average of Iterations) ===")
        print("Cold Start:")
        print(f"  Processing time:          {avg_cold_time:.2f} seconds (min: {min(cold_times):.2f}s, max: {max(cold_times):.2f}s)")
        print(f"  Processing speed:         {avg_cold_speed:,.0f} characters/second")
        
        print("\nWarm Start (With JIT Optimization):")
        print(f"  Processing time:          {avg_warm_time:.2f} seconds (min: {min(warm_times):.2f}s, max: {max(warm_times):.2f}s)")
        print(f"  Processing speed:         {avg_warm_speed:,.0f} characters/second")
        print(f"  Speed improvement:        {speed_improvement:.1f}%")
        
        print("\nDocuments Statistics:")
        print(f"  Documents processed:      {len(test_texts)}")
        print(f"  Total characters:         {test_texts[0][:20]}... (total: {len(test_texts[0]):,})")
...
def main():
    ...
    parser.add_argument(
        "--benchmark", action="store_true", help="Run performance benchmark only"
    )
    parser.add_argument(
