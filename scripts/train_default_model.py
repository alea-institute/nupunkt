...
def print_training_details(trainer: PunktTrainer, initial_abbrevs: set) -> None:
    ...
    if learned_abbrevs:
        ...
        for abbr in sorted(learned_abbrevs)[:20]:
            tqdm.write(f"  - {abbr}")
...
def main() -> None:
    ...
    parser.add_argument(
        "--no-batches", action="store_true", help="Disable batch training mode (on by default)"
    )
    ...
    # Combine them
    abbreviations = sorted(set(legal_abbreviations + general_abbreviations))
